...
class JsonLdEndpointDatasource extends Datasource {
  ...
  _createUriRequest(quadPattern, offset, limit) {
    ...
    if (quadPattern) {
      if (quadPattern.subject) `${uriRequest}&start=${quadPattern.subject}`;
      if (quadPattern.predicate) `${uriRequest}&rel=${quadPattern.predicate}`;
      if (quadPattern.object) `${uriRequest}&end=${quadPattern.object}`;
  ...
  _createQuadPattern(quad) {
    ...
    // Add a possible subject, predicate and object IRIs
    for (const [key, value] of Object.entries(quad)) quadPattern[key] = this._encodeObject(value);
